...
public class SystemTestUtil {
  ...
  public static HeapFile createRandomHeapFile(
      ...
      String colPrefix)
      throws IOException, DbException, TransactionAbortedException {
    return createRandomHeapFile(
        columns, rows, MAX_RAND_VALUE, columnSpecification, tuples, colPrefix);
  }
  ...
  private static double[] getDiff(double[] sequence) {
    ...
    for (int i = 0; i < sequence.length - 1; ++i) ret[i] = sequence[i + 1] - sequence[i];
    return ret;
  }
  /**
  ...
  public static Object[] checkConstant(double[] sequence) {
    ...
    // compute average
    double sum = .0;
    for (int i = 0; i < sequence.length; ++i) sum += sequence[i];
    double av = sum / (sequence.length + .0);
    // compute standard deviation
    double sqsum = 0;
    for (int i = 0; i < sequence.length; ++i) sqsum += (sequence[i] - av) * (sequence[i] - av);
