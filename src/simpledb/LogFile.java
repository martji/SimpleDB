...
/**
 * LogFile implements the recovery subsystem of SimpleDB. This class is able to write different log
 * records as needed, but it is the responsibility of the caller to ensure that write ahead logging
 * and two-phase locking discipline are followed.
 *
 * <p><u> Locking note: </u>
 *
 * <p>
 *
 * <p>Many of the methods here are synchronized (to prevent concurrent log writes from happening);
 * many of the methods in BufferPool are also synchronized (for similar reasons.) Problem is that
 * BufferPool writes log records (on page flushed) and the log file flushes BufferPool pages (on
 * checkpoints and recovery.) This can lead to deadlock. For that reason, any LogFile operation that
 * needs to access the BufferPool must not be declared synchronized and must begin with a block
 * like:
 *
 * <p>
 *
 ...
 *
 * <ul>
 *   <li>The first long integer of the file represents the offset of the last written checkpoint, or
 *       -1 if there are no checkpoints
 *   <li>All additional data in the log consists of log records. Log records are variable length.
 *   <li>Each log record begins with an integer type and a long integer transaction id.
 *   <li>Each log record ends with a long integer file offset representing the position in the log
 *       file where the record began.
 ...
 *   <li>UPDATE RECORDS consist of two entries, a before image and an after image. These images are
 *       serialized Page objects, and can be accessed with the LogFile.readPageData() and
 *       LogFile.writePageData() methods. See LogFile.print() for an example.
 *   <li>CHECKPOINT records consist of active transactions at the time the checkpoint was taken and
 *       their first log record on disk. The format of the record is an integer count of the number
 *       of transactions, as well as a long integer transaction id and a long integer first record
 *       offset for each active transaction.
 * </ul>
 */
public class LogFile {
  ...
  /**
   * Write an UPDATE record to disk for the specified tid and page (with provided before and after
   * images.)
   *
   ...
   * pre-updated state. To preserve transaction semantics, this should not be called on transactions
   * that have already committed (though this may not be enforced by this method.)
   *
