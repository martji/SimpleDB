...
public class TestUtil {
  ...
  /**
   * @return a DbIterator over a list of tuples constructed over the data provided in the
  ...
  public static TupleIterator createTupleList(int width, int[] tupdata) {
    ...
    while (i < tupdata.length) {
      ...
      for (int j = 0; j < width; ++j) tup.setField(j, getField(tupdata[i++]));
      tuplist.add(tup);
    }
  ...
  public static TupleIterator createTupleList(int width, Object[] tupdata) {
    ...
    while (i < tupdata.length) {
      ...
      for (int j = 0; j < width; j++) {
        Field f;
        Object t = tupdata[i++];
        if (t instanceof String) f = new StringField((String) t, Type.STRING_LEN);
        else f = new IntField((Integer) t);
  ...
  public static boolean compareTuples(Tuple t1, Tuple t2) {
    if (t1.getTupleDesc().numFields() != t2.getTupleDesc().numFields()) return false;

    for (int i = 0; i < t1.getTupleDesc().numFields(); ++i) {
      if (!(t1.getTupleDesc().getFieldType(i).equals(t2.getTupleDesc().getFieldType(i))))
        return false;
      if (!(t1.getField(i).equals(t2.getField(i)))) return false;
    }
  ...
  public static byte[] readFileBytes(String path) throws IOException {
    ...
    if (offset < buf.length) throw new IOException("failed to read test data");
  ...
  public static class MockScan implements DbIterator {
    ...
    protected Tuple readNext() {
      ...
      for (int i = 0; i < width; ++i) tup.setField(i, new IntField(cur));
      cur++;
      return tup;
    }
  ...
  public abstract static class CreateHeapFile {
